...
(function () {
    ...
    CKEDITOR.plugins.add("autosave", {
        lang: "bg,ca,cs,de,en,es,eu,fr,it,ja,nl,pl,pt-br,ru,sk,sv,tr,zh,zh-cn",
        ...
        init: function (editor) {
            ...
            var _saveKeyPrefix = "saveKeyPrefix" in editor.config.autosave ? editor.config.autosave.saveKeyPrefix : "autosave", _saveKeyIgnoreProto = "saveKeyIgnoreProtocol" in editor.config.autosave ? editor.config.autosave.saveKeyIgnoreProtocol : false, _saveKeyUrl = _saveKeyIgnoreProto ? window.location.href.replace(/https?:\/\//, "") : window.location.href, _saveKeyDelimiter = "saveKeyDelimiter" in editor.config.autosave ? editor.config.autosave.saveKeyDelimiter : "_", _saveKeyAttribute = "saveKeyAttribute" in editor.config.autosave ? editor.config.autosave.saveKeyAttribute : "name";
            if ("saveKeyIgnoreParams" in editor.config.autosave) {
                CKEDITOR.tools.array.forEach(editor.config.autosave.saveKeyIgnoreParams, function () {
    ...
    function onTimer(configAutosave, editorInstance) {
        if (editorInstance.checkDirty() || editorInstance.plugins.bbcode) {
            var editor = editorInstance, autoSaveKey = configAutosave.SaveKey != null
    ...
    function CheckForAutoSavedContent(editorInstance, config, autoSaveKey, notOlderThen) {
        ...
        if (localStorage.getItem(autoSaveKey)) {
            ...
            if (config.autoLoad) {
                ...
                var confirmMessage = editorInstance.lang.autosave.loadSavedContent.replace("{0}", moment(autoSavedContentDate).locale(editorInstance.config.language)
